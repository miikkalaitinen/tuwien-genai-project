    ...
    <div className="bg-white rounded-xl shadow-2xl p-8 max-w-xl w-full relative border border-gray-300">
      ...
      <div className="mb-6">
        ...
        <div className="mb-4 flex flex-col gap-2">
          <div>
            <span className="font-semibold text-blue-700">Paper A (Origin):</span> <span className="text-gray-800">{nodes.find(n => n.id === selectedEdge.source)?.data?.label || selectedEdge.source}</span>
          </div>
          <div>
            <span className="font-semibold text-green-700">Paper B (Target):</span> <span className="text-gray-800">{nodes.find(n => n.id === selectedEdge.target)?.data?.label || selectedEdge.target}</span>
          </div>
        </div>
        <div className="mb-3">
